...
public class ExtraBallPower extends PowerUp
{
	...
	gameBoard.getBalls()
		.add(new NormalBall(gameBoard.getBalls().get(0).getX() + xOffset, gameBoard.getBalls().get(0).getY() + yOffset));
	gameBoard.incrementNumberOfBalls();
	gameBoard.getBalls().get(gameBoard.getNumberOfBalls() - 1).flipYDir(); // Flip the y-direction of the new ball
